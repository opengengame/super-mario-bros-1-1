...
HF_HOME=/mnt/store/kmei1/HF_HOME torchrun --master_port 29502 --nnodes=1 \
    --node_rank=0 \
    --nproc_per_node=8 \
...
def main(args):
    ...
    opt = torch.optim.AdamW(model.parameters(), lr=configs.learning_rate)
    # opt = bnb.optim.Adam8bit(model.parameters(), lr=configs.learning_rate, betas=(0.9, 0.995))
        ...
        for epoch in range(args.epochs):
            ...
            for data in loader:
                ...
                with torch.autocast(device_type='cuda'):
                    ...
                    # print("%5f %5f %5f %5f" % (target.max(), target.min(), model_pred.max(), model_pred.min()))
                    loss = torch.mean((target.detach() - model_pred) ** 2)
