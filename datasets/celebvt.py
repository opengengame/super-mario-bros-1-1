...
class Dataset(VideoFolder):
    def __init__(
        ...
        super().__init__(path, size=size, nframes=nframes)

        general_txt = os.listdir(
            "/data/kmei1/datasets/CelebV-Text/texts/face40_details_new"
        )
        general_txt.sort()

        emotion_txt = os.listdir("/data/kmei1/datasets/CelebV-Text/texts/emotion")
        emotion_txt.sort()

        action_txt = os.listdir(
            "/data/kmei1/datasets/CelebV-Text/texts/action_dur"
        )
        action_txt.sort()

        videos = os.listdir(path)
        videos = [x[:-4] + ".txt" for x in videos]
        videos.sort()

        _video_fnames = list(set(videos) & set(general_txt) & set(action_txt) & set(emotion_txt))
        _video_fnames.sort()
    ...
    def __getitem__(self, index):
        ...
        video_path = os.path.join(self.path, self._video_fnames[index][:-4] + ".mp4")
        ...
        with open(
            f"/data/kmei1/datasets/CelebV-Text/texts/face40_details_new/{self._video_fnames[index]}"
        ) as f:
            general_content = f.read().splitlines()[0]
        with open(
            f"/data/kmei1/datasets/CelebV-Text/texts/action_dur/{self._video_fnames[index]}"
        ) as f:
            action_content = f.read().splitlines()[0]
        with open(
            f"/data/kmei1/datasets/CelebV-Text/texts/emotion/{self._video_fnames[index]}"
        ) as f:
            emotion_content = f.read().splitlines()[0]
        ...
        return (video, general_content + " " + emotion_content + " " + action_content , grid)

    def __len__(self):
        return self._total_size * 1000
